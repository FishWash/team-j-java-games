...
//      System.out.println("(PlayerCamera) Red: " + red + " Green: " + green);
    ...
    if(displayText == DisplayText.Win){
      graphics2D.drawString("You Win!", (playerDisplay.getWidth() / 2) - fontMetrics.stringWidth("You Win!") / 2, playerDisplay.getHeight() / 2);
    } else if (displayText == DisplayText.Lose){
      graphics2D.drawString("You Lose.", () - fontMetrics.stringWidth("You Lose.") / 2, screenHeight / 2 );
    } else if(displayText == DisplayText.Draw){
      graphics2D.setColor(Color.WHITE);
      graphics2D.drawString("Draw",(screenWidth / 4) - fontMetrics.stringWidth("Draw") / 2, screenHeight / 2);
    }
